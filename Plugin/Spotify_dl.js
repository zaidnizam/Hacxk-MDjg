...
module.exports = (Command) => {
    Command({
        ...
        react: "🎼",
        type: 'SPOTIFY DOWNLOADER COMMANDS',
        handler: async (m, sock) => {
            ...
            try {
                if (input.match(/(https:\/\/www\.spotify\.com\/|https:\/\/open\.spotify\.com\/)/)) {
                    const result = await axios.get(`https://api.junn4.my.id/download/spotify?url=${input}`); // Wait for the result of the GET request 
                    await sock.sendMessage(m.key.remoteJid, { react: { text: "🔃", key: m.key } });
                    const dlLink = result.data.data.download
                    const title = result.data.data.title
                    await downloadSpotifySong(sock, m, dlLink, title)
                } else {
                    const url = `https://api.junn4.my.id/search/spotify?query=${input}`
